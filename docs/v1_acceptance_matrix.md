...
| FR-013 | 垂类适配器与工作流分发 | 通过 vertical registry/adapter 查找对应 workflow；新增 vertical 时不复制整套系统 | N/A（架构） | N/A | Vertical Adapter | Milestone 2 | P1 | N/A | 新增 vertical 只需新增 registry/adapter 配置 | 已完成 | `src/vertical_registry.py` 已创建，`VerticalAdapter` 接口已文档化；垂类工作流分发逻辑已完成 |
| FR-014 | 前端垂类状态管理 | 前端状态包含 selectedVertical；UI 显示 content_platform、content_type、vertical；不再将文案写死为"美甲" | N/A（前端状态） | Studio/All | N/A | Milestone 2 | P1 | 已覆盖 | 页面显示当前 vertical；切换 vertical 时状态同步 | 已完成 | `platform_labels` 和 `content_type_labels` 已添加到 `VerticalDefinition`；前端 `labelForPlatform()`/`labelForContentType()` 现使用 vertical-specific 映射表 |
| FR-015 | 兼容旧接口与旧数据 | 旧 /api/nail/... 作为兼容层保留；旧 package fallback 推断 vertical；兼容层不得绕过新安全规则 | /api/nail/... 兼容层 | N/A | 兼容适配层 | Milestone 1 | P1 | 已覆盖 | 旧接口仍可工作；旧数据可 fallback | 已完成 | `POST /api/verticals/{vertical}/notes` 现已实现；前端已从硬编码 `/api/nail/notes` 迁移至动态 URL |
...
| 2026-05-03 | Codex/Hermes | FR-013 垂类适配器与工作流分发已完成：`src/vertical_registry.py` 已创建，`VerticalAdapter` 接口已文档化；commit: 0a1a9d8 |
| 2026-05-03 | Codex/Hermes | FR-014 前端垂类状态管理已完成：`platform_labels` 和 `content_type_labels` 已添加到 `VerticalDefinition`；前端 `labelForPlatform()`/`labelForContentType()` 现使用 vertical-specific 映射表；commit: 2b67828 |
| 2026-05-03 | Codex/Hermes | FR-015 兼容旧接口与旧数据已完成：`POST /api/verticals/{vertical}/notes` 现已实现；前端已从硬编码 `/api/nail/notes` 迁移至动态 URL；commit: e505c78 |
